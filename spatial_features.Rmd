...
params:
    ...
    tcga_bagaev_subtypes: bagaev_file
    tcga_survival: tcga_survival_file
...
# survival_tcga_liu: "./data/mmc6.xlsx",  tcga_survival: "./data/mmc1.xlsx"
...
bagaev_file = "./data/MFP.xlsx"
# params$tcga_bagaev_subtypes
bagaev_mfp <- readxl::read_xlsx(path = bagaev_file, sheet = 1, skip = 1)
bagaev_mfp <- bagaev_mfp %>% filter(Cohort == "TCGA-SKCM")
colnames(bagaev_mfp)[1] <- "TCGA_patient_ID"

# Read Survival data for TCGA data -------------------------------------------
tcga_survival_file = "./data/survival.xlsx"
# params$tcga_survival
survival_tcga_liu <- readxl::read_xlsx(path = tcga_survival_file, sheet = 1)
...
cell_prob <- data.table::fread(
  file = file.path(params$spatial_features_dir, "tcga_validation_tile_predictions_proba.csv"), 
                   sep = "\t")

## Pivot to long df
cell_prob <- cell_prob %>% 
  select(slide_submitter_id, CAFs, T_cells, endothelial_cells, tumor_purity, Coord_X, Coord_Y) %>%
  pivot_longer(cols = !c(slide_submitter_id, Coord_X, Coord_Y), 
...
ggarrange(gg_maps[[1]], 
          gg_maps[[2]], 
          gg_maps[[3]], 
          gg_maps[[4]], nrow = 1, ncol = 4, align = "h", common.legend = TRUE)
...
cell_prob_mean$MFP <- factor(cell_prob_mean$MFP, levels=c("D", "F", "IE/F", "IE"))
...
cell_prob_mean$celltype <- factor(cell_prob_mean$celltype, 
                                  levels = c("CAFs", "endothelial_cells", 
                                             "T_cells", "tumor_purity"))

## Plot
bxp_prob <- ggpubr::ggboxplot(
  cell_prob_mean, x = "celltype", y = "mean_prob",
...
cell_coloc$MFP <- factor(cell_coloc$MFP, levels=c("D", "F", "IE/F", "IE"))

cell_coloc$pair <- factor(cell_coloc$pair, levels = c("CAF-Endo", "CAF-Tcell", "CAF-Tumor",
...
bxp_coloc <- ggpubr::ggboxplot(
  cell_coloc, x = "pair", y = "frac",
  fill = "MFP", color = "MFP", alpha = 0.8, palette = colors_mfp,
  add = "jitter", add.params = list(size = 0.8, jitter = 0.08)
  ) +
  theme_minimal() +
  theme(axis.text = element_text(size=16, angle = 45, hjust = 1, color = "black"),
        axis.title.x = element_blank(),
        axis.title.y = element_text(size=16, color = "black"), 
        legend.position = "top", legend.direction = "horizontal", 
        legend.title = element_text(size=16, color = "black"),
...
ggarrange(fig2a, 
          fig2b, nrow = 1, ncol = 2, align = "h", common.legend = TRUE)
...
newlevels <- c("CAF-CAF (L-H)", "CAF-CAF (H-H)",
...
pt_1 <- ggarrange(Fig4a, 
                  Fig4b, nrow = 1, ncol = 2, align = "v", widths = c(1, 1),
                  common.legend = TRUE)

## Combine last panel
ggarrange(pt_1, 
          Fig4c, nrow = 2, ncol = 1, align = "v", widths = c(0.5, 1),
          legend = "none")
...
colors <- c("darkgreen", "darkblue", "darkorange", "salmon", "darkred",
            "goldenrod", "gray")

names(colors) <- c("CAF", "Tcell", "CAF&Tcell", "Tcell&Tumor", "Tumor", 
...
ggarrange(gg_maps[[1]], 
          gg_maps[[2]], 
          gg_maps[[3]], 
          gg_maps[[4]], nrow = 1, ncol = 4, align = "h", common.legend = FALSE)
...
cox_features_df <- do.call(rbind, lapply(colnames(tcga_survival_mat)[1:96], function(X){
  ...
  cox_df <- cox_fit %>% tidy() %>% dplyr::mutate(term = ifelse(term == "get(X)", X, term))
...
#pROC::auc(roc(CPTAC_1yearsurv, test_pred_median))
...
summary_coeff <- data.frame(name=recurrent_features,
...
coef_plot <- ggplot(data=summary_coeff, aes(x=reorder(name, -median), y=median)) +
  ...
  ylim(c(-2.5,2.5)) +
  coord_flip()+ 
  theme(axis.text.x = element_text(hjust = 1, size = 12, color = "black", angle = 45),
